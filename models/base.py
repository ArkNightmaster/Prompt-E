...
class BaseLearner(object):
    def __init__(self, args):
        ...
        self._data_memory, self._targets_memory, self._names_memory = np.array([]), np.array([]), np.array([])
        self.topk = 5
    ...
    def feature_dim(self):
        if isinstance(self.network1, nn.DataParallel):
            return self.network1.module.feature_dim
        else:
            return self.network1.feature_dim
    ...
    def umap(self,showcenters=False,Normalize=False):
        ...
        if showcenters:
            fc_weight= self.network1.fc.proj.cpu().detach().numpy()[:tot_classes]
    ...
    def save_checkpoint(self, filename):
        ...
        save_dict = {
            ...
            "model_state_dict": self.network1.state_dict(),
        }
    ...
    def _get_memory(self):
        if len(self._data_memory) == 0:
            ...
            return (self._data_memory, self._targets_memory, self._names_memory)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets, _) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                outputs = self.network1(inputs)["logits"]
            ...
            for _, _inputs, _targets, _ in loader:
                ...
                if isinstance(self.network1, nn.DataParallel):
                    _vectors = tensor2numpy(
                        self.network1.module.extract_vector(_inputs.to(self._device))
                    )
                else:
                    _vectors = tensor2numpy(
    ...
    def _reduce_exemplar(self, data_manager, m):
        ...
        dummy_data, dummy_targets, dummy_names = copy.deepcopy(self._data_memory), copy.deepcopy(self._targets_memory), copy.deepcopy(self._names_memory)
        self._class_means = np.zeros((self._total_classes, self.feature_dim))
        self._data_memory, self._targets_memory, self._names_memory = np.array([]), np.array([]), np.array([])

        for class_idx in range(self._known_classes):
            mask = np.where(dummy_targets == class_idx)[0]
            dd, dt, dn = dummy_data[mask][:m], dummy_targets[mask][:m], dummy_names[mask][:m]
            self._data_memory = (
            ...
            self._names_memory = (
                np.concatenate((self._names_memory, dn))
            ...
            idx_dataset = data_manager.get_dataset(
                [], source="train", mode="test", appendent=(dd, dt, dn)
            )
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            class_data, class_targets, class_names = (
                self._data_memory[mask],
                self._targets_memory[mask],
                self._names_memory[mask]
            )

            class_dset = data_manager.get_dataset(
                [], source="train", mode="test", appendent=(class_data, class_targets, class_names)
            )
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            data, targets, names, class_dset = data_manager.get_dataset(
                np.arange(class_idx, class_idx + 1),
                source="train",
                mode="test",
                ret_data=True,
            ...
            selected_exemplars = np.array(selected_exemplars)
            exemplar_targets = np.full(m, class_idx)
            exemplar_names = np.full(m, names[0])
            self._data_memory = (
                np.concatenate((self._data_memory, selected_exemplars))
                if len(self._data_memory) != 0
                else selected_exemplars
            ...
            self._names_memory = (
                np.concatenate((self._names_memory, exemplar_names))
                if len(self._names_memory) != 0
                else exemplar_names
            )
            ...
            exemplar_dset = data_manager.get_dataset(
                ...
                source="train",
                mode="test",
                appendent=(selected_exemplars, exemplar_targets, exemplar_names),
            )
