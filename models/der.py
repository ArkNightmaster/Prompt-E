...
class Learner(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self.network1 = DERNet(args, pretrained=args['pretrained'])
    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task > 0:
            for i in range(self._cur_task):
                for p in self.network1.backbones[i].parameters():
                    p.requires_grad = False

        logging.info("All params: {}".format(count_parameters(self.network1)))
        logging.info(
            "Trainable params: {}".format(count_parameters(self.network1, True))
        )
    ...
    def train(self):
        self.network1.train()
        if len(self._multiple_gpus) > 1 :
            self._network_module_ptr = self.network1.module
        else:
            self._network_module_ptr = self.network1
