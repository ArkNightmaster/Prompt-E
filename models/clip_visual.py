...
from utils.inc_net import CLIP_Encoder
...
class Learner(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        model, _ = clip.load("ViT-B/16", device=self._device, download_root="./clip/download",
                             ...
                             use_lrg = args["use_lrg"],
                             mask_ratio = args["mask_ratio"],
                             num_classes = args["nb_classes"]
                             )
        model = model.float()
        self.model = model
        self.network1 = model.visual # 载入使用了prompt-pool的clip-visual
        model, _ = clip.load("ViT-B/16", device=self._device, download_root="./clip/download", 
                             original=args["get_original_backbone"],
                             use_clip_proj=False
                             )
        model = model.float()
        self.original_backbone = model.visual
        ...
        self.task_features = {}  # 存储每个任务的语言特征
        self.train_classes = []
        ...
        if self.args["freeze"]:
            ...
            # freeze args.freeze ["positional_embedding", "class_embedding", "proj", "conv1", "ln_pre", "ln_post", "transformer"] parameters
            for n, p in self.network1.named_parameters():
    ...
    def incremental_train(self, data_manager):
        ...
        # ————————————————————————————————————
        self.train_map = order_num2label(train_dataset.labels, train_dataset.names)  # 获取pairs of 数字：标签
        self.train_classes_temp = [self.train_map[i + self._known_classes] for i in range(len(self.train_map))]  # 获取标签顺序列表
        self.train_classes.extend(self.train_classes_temp)

        # ————————————————————————————————————
        # Load Text Encoder
        # ————————————————————————————————————
        if "clip" in self.text_encoder:
    ...
    def _train(self, train_loader, test_loader):
        ...
        self.original_backbone.to(self._device)
        ...
        if self._cur_task > 0:
            # 当存在共享prompt池时进行prompt初始化操作，默认不进行该步
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets, names) in enumerate(train_loader):
                ...
                with torch.cuda.amp.autocast():
                    cls_feature = self.original_backbone(inputs) # Batch, 512
                    # 获取当前任务的text_embedding
                    text_embedding = self.text_feature
                    # cur_text = self.text_feature[targets]
                    output = self.network1(inputs, task_id=self._cur_task, cls_features=cls_feature, train=True)
                    
                    # 计算图像特征和文本特征之间的余弦相似度
                    logits = output["logits"][:, :self._total_classes]  # [batch_size, 10]
                    logits[:, :self._known_classes] = float('-inf')
                    # 适用余弦相似度来分类
                    # image_features = output["logits"]  # [batch_size, 512]
                    # text_features = text_embedding     # [total_classes, 512]
                    # # 归一化特征
                    # image_features = F.normalize(image_features, dim=1)
                    # # 计算余弦相似度
                    # similarity = image_features @ text_features.T  # [batch_size, total_classes]
                    # # 将未见过的类别的相似度设为负无穷
                    # similarity[:, :self._known_classes] = float('-inf')

                    
                    # _, preds_target = torch.max(similarity, dim=1)
                    # preds = torch.softmax(similarity, dim=1)

                    # 计算交叉熵损失
                    loss_origin = F.cross_entropy(logits, targets)
                    
                    # 其他损失计算保持不变
                    if self.args["use_lrg"]:
                        loss_text_matching = info_nce_loss(output['reconstruct_pre_logits'], text_feature, targets, tau=self.tau)
                    else:
                        loss_text_matching = torch.tensor(0.0)
            ...
            if (epoch + 1) % 5 == 0:
                ...
                info = "Task {}, Epoch {}/{} => Loss {:.3f}, Loss_Origin{:.3f}, Loss_Text_Matching{:.3f}, Train_accy {:.2f}, Test_accy {:.2f}".format(
                    self._cur_task,
                    epoch + 1,
                    self.args['epochs'],
                ...
                info = "Task {}, Epoch {}/{} => Loss {:.3f}, Loss_Origin{:.3f}, Loss_Text_Matching{:.3f}, Train_accy {:.2f}".format(
                    self._cur_task,
                    epoch + 1,
                    self.args['epochs'],
                    losses / len(train_loader),
    ...
    def _eval_cnn(self, loader):
        ...
        for i, (_, inputs, targets, names) in enumerate(loader):
            ...
            with torch.no_grad():
                # cur_text = self.text_feature[targets]
                # 获取cls_feature
                cls_feature = self.original_backbone(inputs)
                # 获取图像特征
                logits = self.network1(inputs, task_id=self._cur_task, cls_features=cls_feature)["logits"][:, :self._total_classes]
                
                # # 使用余弦相似度来分类
                # # 获取文本特征
                # text_features = self.text_feature[:self._total_classes]
                # # 计算相似度
                ...
                # Save Prompt Weights
                if i % 10 == 0 and self.args['rt_weight']:
                    prompt_weights = tensor2numpy(self.clip_visual(inputs, task_id=self._cur_task)["prompt_weight"])
                    np.save('./fig/prompt_weights/{}/{}/top{}/Task_{}_batch_{}_prompt_weights.npy'.format(self.args['model_name'], self.args['dataset'], self.args['top_k'], self._cur_task, i), prompt_weights)
    ...
    def _compute_accuracy(self, model, loader):
        ...
        for i, (_, inputs, targets, names) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                cls_feature = self.original_backbone(inputs)
                # 获取图像特征
                logits = model(inputs, task_id=self._cur_task, cls_features=cls_feature)["logits"][:, :self._total_classes]
    ...
    def set_training(self, mode=True):
        self.network1.train(True)
        self.original_backbone.train(False)