...
class Generalized_Vit(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, global_pool='token', embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, init_values=None, class_token=True, no_embed_class=False, pre_norm=False, fc_norm=None, drop_rate=0, attn_drop_rate=0, drop_path_rate=0, weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block):
...
class Specialized_Vit(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, global_pool='token', embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, init_values=None, class_token=True, no_embed_class=False, pre_norm=False, fc_norm=None, drop_rate=0, attn_drop_rate=0, drop_path_rate=0, weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block):
        super().__init__(img_size, patch_size, in_chans, num_classes, global_pool, embed_dim, depth, num_heads, mlp_ratio, qkv_bias, init_values, class_token, no_embed_class, pre_norm, fc_norm, drop_rate, attn_drop_rate, drop_path_rate, weight_init, embed_layer, norm_layer, act_layer, block_fn)
    ...
    def forward(self, base_feature_map):
        x = self.last_block(base_feature_map)
        x = self.norm(x)
        x = self.forward_head(x)
        return x

def _create_vision_transformer_base(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
def _create_vision_transformer_adaptive(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
def get_memo_vit(pretrained, kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    base_model = _create_vision_transformer_base('vit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    adaptive_model = _create_vision_transformer_adaptive('vit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return base_model, adaptive_model

@register_model
def vit_base_patch16_224_memo(pretrained=False, **kwargs):
