...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            mask_ratio=0.75,rt_weight=False):
        """
        Args:
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        # Language Representation Guidance (LRG) Branch
        x2 = res['cls_token_logits']
        x2 = x2.unsqueeze(2)  # 16,768,1
        x2 = self.downstream_layers(x2)  # 16,768,1
        x2 = x2.squeeze(2)  # 16,768
        res['reconstruct_pre_logits'] = x2  # 16,768
...
def resize_pos_embed_clip(posemb, posemb_new, num_prefix_tokens=1, gs_new=()):
    """调整位置嵌入的大小，处理 CLS token 和提示标记
    Args:
        posemb: 原始位置嵌入 [1, N, D]
        posemb_new: 目标位置嵌入 [1, N_new, D]
        num_prefix_tokens: 特殊标记数量（如 CLS token）
        gs_new: 新的网格大小 (H_new, W_new)
    """
    _logger.info('Resized position embedding: %s to %s', posemb.shape, posemb_new.shape)
    
    # 获取维度信息
    ntok_new = posemb_new.shape[0]  # 新的序列长度
    dim = posemb.shape[-1]      # 嵌入维度

    # 分离 CLS token 和网格位置嵌入
    if num_prefix_tokens:
        posemb_prefix = posemb[:num_prefix_tokens]  # [1, D]
        posemb_grid = posemb[num_prefix_tokens:]    # [N-1, D]
    else:
        posemb_prefix = None
        posemb_grid = posemb    # [N, D]
    ...
    # 如果未指定新的网格大小，则计算
    if not len(gs_new):
        gs_new = [int(math.sqrt(ntok_new - num_prefix_tokens))] * 2
    assert len(gs_new) >= 2

    # 调整网格位置嵌入
    posemb_grid = posemb_grid.reshape(gs_old, gs_old, -1)  # [H, W, D]
    posemb_grid = posemb_grid.permute(2, 0, 1)  # [D, H, W]
    posemb_grid = posemb_grid.unsqueeze(0)  # [1, D, H, W]
    posemb_grid = F.interpolate(posemb_grid, size=gs_new, mode='bicubic', align_corners=False)
    posemb_grid = posemb_grid.squeeze(0)  # [D, H_new, W_new]
    posemb_grid = posemb_grid.permute(1, 2, 0)  # [H_new, W_new, D]
    posemb_grid = posemb_grid.reshape(-1, dim)  # [H_new*W_new, D]

    if num_prefix_tokens:
        if ntok_new > (gs_new[0] * gs_new[1] + num_prefix_tokens):
            n_add = ntok_new - (gs_new[0] * gs_new[1] + num_prefix_tokens)
            extra_tokens = posemb_prefix.repeat(n_add, 1)  # [n_add, D]
            posemb_grid = torch.cat([posemb_grid, extra_tokens], dim=0)  # [H_new*W_new + n_add, D]
        
        posemb = torch.cat([posemb_prefix, posemb_grid], dim=0)  # [N_new, D]
    else:
        posemb = posemb_grid

    if len(posemb_new.shape) == 3:
        posemb = posemb.unsqueeze(0)  # [1, N_new, D]
...
@register_model
def vit_tiny_patch16_224_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
...
@register_model
def vit_tiny_patch16_384_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch32_224_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/32)
    """
...
@register_model
def vit_small_patch32_384_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_224_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
...
@register_model
def vit_small_patch16_384_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    NOTE I've replaced my previous 'small' model definition and weights with the small variant from the DeiT paper
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
...
@register_model
def vit_base_patch32_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch32_384_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_384_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch8_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch32_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch32_384_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch16_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch16_384_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
...
@register_model
def vit_large_patch14_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_huge_patch14_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_giant_patch14_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_gigantic_patch14_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_tiny_patch16_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16).
...
@register_model
def vit_small_patch32_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_small_patch16_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch32_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch16_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch8_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch32_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
...
@register_model
def vit_large_patch16_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_huge_patch14_224_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has a representation layer but the 21k classifier head is zero'd out in original weights
    """
...
@register_model
def vit_base_patch16_224_sam_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch32_224_sam_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_224_dino_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch8_224_dino_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_dino_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch8_224_dino_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_224_miil_in21k_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
...
@register_model
def vit_base_patch16_224_miil_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False, **kwargs)
...
@register_model
def vit_base_patch32_plus_256_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/32+)
    """
...
@register_model
def vit_base_patch16_plus_240_clip_prompt_l2p(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16+)
    """
...
@register_model
def vit_base_patch16_rpn_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_36x1_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_18x2_224_clip_prompt_l2p(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_18x2_224_clip_prompt_l2p(pretrained=False, **kwargs):
