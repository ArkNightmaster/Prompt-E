...
class align_layers(nn.Module):
    ...
    """
    def __init__(self, embed_dim, text_dim, mask_ratio):
        super().__init__()
        self.lambda1 = mask_ratio
        self.align_layer = nn.Conv1d(in_channels=embed_dim, out_channels=text_dim, kernel_size=1, stride=1, padding=0)
    def forward(self, x):
...
class LRG_layer(nn.Module):

    def __init__(self, embed_dim, text_dim, mask_ratio):
        super().__init__()
        self.align_layer = align_layers(embed_dim, text_dim, mask_ratio)
        self.generative_linear = nn.Sequential(
            nn.Conv1d(in_channels=text_dim, out_channels=text_dim, kernel_size=1, stride=1, padding=0),
            nn.ReLU(),
            nn.Conv1d(in_channels=text_dim, out_channels=text_dim, kernel_size=1, stride=1, padding=0))
    
    def forward(self, x):
        x = self.align_layer(x)
        x = self.generative_linear(x)
        return x
...
# coda_prompt + Prompt-E
class CodaPrompt_PE(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768, mask_ratio=0.75):
        super().__init__()
        self.task_count = 0
        self.emb_d = emb_d
        self.key_d = key_dim
        ...
        self.LRG_layers = nn.ModuleList([LRG_layer(key_dim, text_dim=512, mask_ratio=mask_ratio) for _ in range(len(self.e_layers))])
...
# lgcl prompt
class EPrompt_LGCL(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False, 
                 prompt_key=False, pool_size=None, top_k=None, batchwise_prompt=False, prompt_key_init='uniform',
                 num_layers=1, use_prefix_tune_for_e_prompt=False, num_heads=-1, same_key_value=False,args=None,clip_model=None, class_names=None,natural_cls=None, class_mask= None,task_cls =None):
        super().__init__()
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None, task_id = -1 ):
        ...
        if self.prompt_pool:
            ...
            # calculating task level language guidance loss
            total_loss = 0
            task_reps = self.task_rep/self.task_rep.norm(dim=-1,keepdim=True)
            task_rep_b = task_reps[task_id].unsqueeze(0).unsqueeze(0).tile((batched_key_norm.shape[0],1,1))
            positive_rep = batched_key_norm * task_rep_b
            if task_id == 0:
                total_loss = 1 - (torch.sum(positive_rep)/x_embed.shape[0])
            else:
                neg_rep_indices = [random.randint(0,task_id-1) for _ in range(batched_key_norm.shape[0])]
                
                neg_rep = task_reps[neg_rep_indices].unsqueeze(1) #b,1,C
                
                neg_rep = batched_key_norm * neg_rep
                neg_rep = torch.sum(neg_rep)/ x_embed.shape[0]
                positive_scaler_rep = (torch.sum(positive_rep)/x_embed.shape[0])
                total_loss = 1 - positive_scaler_rep + neg_rep
            out['task_map_loss']= total_loss
            # 
            # Put pull_constraint loss calculation inside
            x_embed_norm = x_embed_norm.unsqueeze(1) # B, 1, C
            sim = batched_key_norm * x_embed_norm # B, top_k, C
...
class Prompt(nn.Module):
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            # 通过计算该批次最相似并且出现次数最多的索引（前N位）的value作为prompt
            # 使该批次都是用相同的索引
